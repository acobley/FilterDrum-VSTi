//------------------------------------------------------------------------
// FilterDrum - the bar clock
//
// LIFTED WHOLE from ~/DXi-DEv/Project6-VSTi/source/Project6Transport.*,
// with two deliberate changes and nothing else: the NAMESPACE, and the
// grid runs at SIXTEENTHS rather than eighths so that one grid line is
// one step of this plug-in's sixteen-step sequencer. LaunchDivision
// gains a Sixteenth for the same reason.
//
// Everything below is Project6's, comments included, so a `diff` against
// it shows only those two changes and a fix made in either can be
// carried across by hand. tests/TransportTests.cpp came with it -
// lifting the arithmetic and leaving its tests behind would have been
// exactly the wrong half to take.
//
...
// tested in tests/TransportTests.cpp. FilterDrumProcessor's job is only to
...
//
//     1/1  the bar line          1/8  an eighth of it
//     1/2  half way through      1/16 a sixteenth - every step
//     1/4  a quarter of the bar
//
...
constexpr int kLaunchDivisionCount = 5;

/** The finest division, and so how many steps a bar is cut into.

    SIXTEEN, WHERE PROJECT6 HAD EIGHT, and that is the whole reason this
    file was lifted rather than shared. One grid line is one step of the
    sequencer, so the sequencer needs no clock of its own: the step index
    that comes back from gridLinesInBlock IS the switch to look up. */
constexpr int kGridStepsPerBar = 16;

/** How many of those steps make one of these. */
constexpr int divisionSteps (LaunchDivision division)
{
	return (division == LaunchDivision::Bar)     ? 16
	     : (division == LaunchDivision::HalfBar) ? 8
	     : (division == LaunchDivision::Quarter) ? 4
	     : (division == LaunchDivision::Eighth)  ? 2
	                                             : 1;
}
...
constexpr LaunchDivision divisionFromIndex (int index)
{
	return (index == 1) ? LaunchDivision::HalfBar
	     ...
	     : (index == 4) ? LaunchDivision::Sixteenth
	                    : LaunchDivision::Bar;
}
