//------------------------------------------------------------------------
// FilterDrum - bar clock tests
//
// SDK-FREE.
//
//     c++ -std=c++17 -O2 -Wall -Isource tests/TransportTests.cpp
//         source/FilterDrumTransport.cpp -o /tmp/transporttests
//         && /tmp/transporttests
//
...
int main ()
{
	std::printf ("FilterDrum - bar clock tests\n");
	...
	//--------------------------------------------------------------------
	{
		...
		{
			BarClock clock;
			// 3600 samples from 3.85 ends exactly on quarter note 4.0,
			// and at sixteenths there is no line between the two. (It
			// was 3.6 / 9600 on the eighth-note grid; that block now
			// contains the line at 3.75.)
			const int n = clock.gridLinesInBlock (rolling (3.85), 3600, kRate,
			                                      lines, kMaxGridLinesPerBlock);
		...
		{
			BarClock clock;
			// 12000 samples from 3.99: spans the bar line at 4.0 and the
			// 1/16 line at 4.25, and stops short of 4.5.
			const int n = clock.gridLinesInBlock (rolling (3.99), 12000, kRate,
			                                      lines, kMaxGridLinesPerBlock);
			check (n == 2, "a block spanning two lines finds both");
			check (n == 2 && lines[0].offset == 240 && lines[1].offset == 6240,
			       "in order, at the right samples");
			check (n == 2 && lines[0].step == 0 && lines[1].step == 1,
			       "and step 0 is followed by step 1");
		}

		// THE STEPS RUN 0..15 AND WRAP. Step 0 is the bar line, and it has
		// to keep being the bar line however far into the project we are.
		{
			...
			const int n = clock.gridLinesInBlock (rolling (40.0 + 2.5 - 0.01), 512, kRate,
			                                      lines, kMaxGridLinesPerBlock);
			check (n == 1 && lines[0].step == 10,
			       "a line two and a half beats into bar 11 is step 10");
		}

		// Odd time signature, so the arithmetic is not silently 4/4. A 7/8
		// bar is 3.5 quarter notes, so a step is 0.21875 - 5250 samples.
		{
			BarClock clock;
			const int n = clock.gridLinesInBlock (rolling (3.0, 7, 8), 24000, kRate,
			                                      lines, kMaxGridLinesPerBlock);
			check (n >= 3, "7/8 has its own step length, and finds lines by it");
			check (n >= 3 && lines[0].offset == 1500 && lines[0].step == 14,
			       "a late step of the bar comes first here");
			check (n >= 3 && lines[2].offset == 12000 && lines[2].step == 0,
			       "and the bar line at 3.5 quarter notes is step 0");

			// NEGATIVE CONTROL for the whole nesting idea: the bar line of
			// a 7/8 bar is a grid line, so a slot on 1/1 and a slot on
			// 1/16 launch together there.
			check (n >= 3 && divisionFires (LaunchDivision::Bar, lines[2].step)
			           && divisionFires (LaunchDivision::Sixteenth, lines[2].step),
	...
	//--------------------------------------------------------------------
	{
		...
		// (see FilterDrumProcessor: it only acts on slots whose armed state
		...
		// exactly once.
		{
			...
			// line: sixteenth lines at 4.0, 4.25 ... 12.5.
			check (fired == 35, "thirty-five lines in eight and a half beats, each fired once");
		}
	...
	//--------------------------------------------------------------------
	{
		check (kGridStepsPerBar == 16, "the grid is sixteen steps to the bar");
		check (kLaunchDivisionCount == 5, "and there are five divisions to choose from");

		check (divisionSteps (LaunchDivision::Bar)     == 16, "a bar is sixteen steps");
		check (divisionSteps (LaunchDivision::HalfBar) == 8, "a half bar is eight");
		check (divisionSteps (LaunchDivision::Quarter) == 4, "a quarter is four");
		check (divisionSteps (LaunchDivision::Eighth)  == 2, "an eighth is two");
		check (divisionSteps (LaunchDivision::Sixteenth) == 1, "and a sixteenth is one - every step");
		...
		for (int step = 0; step < kGridStepsPerBar; ++step)
		{
			...
			sixteenthCount += divisionFires (LaunchDivision::Sixteenth, step) ? 1 : 0;
		}
		...
		check (eighthCount == 8,     "an eighth eight times");
		check (sixteenthCount == 16, "and a sixteenth on every step");

		check (! divisionFires (LaunchDivision::Bar, 8),
		       "NEGATIVE CONTROL: a bar slot does NOT launch half way through");
		check (divisionFires (LaunchDivision::HalfBar, 8),
