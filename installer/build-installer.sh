#!/bin/bash
#-----------------------------------------------------------------------------
# Build a distributable .pkg installer for FilterDrum.
#
...
#
# The result is installer/FilterDrum-<version>.pkg, which installs
#
...
NAME="FilterDrum"
VST3_ID="audio.filterdrum.vst3"
AU_ID="audio.filterdrum.audiounit"
PRESETS_ID="audio.filterdrum.presets"
...
#
# AND THE .aupreset FILES MUST MATCH THEIR .vstpreset. They are generated by
# tools/make-presets.py; shipping one that is stale would give the two formats
# different sounds under the same name. --check refuses rather than
# regenerating silently, because a generated file that changes during a
# release build is a change nobody reviewed.
#-----------------------------------------------------------------------------
PRESET_COUNT=0
if [ -d "$HERE/presets" ]; then
    PRESET_COUNT=$(find "$HERE/presets" -maxdepth 1 -type f \( -name '*.vstpreset' -o -name '*.aupreset' \) | wc -l | tr -d ' ')
fi

if [ "$PRESET_COUNT" -gt 0 ]; then
    command -v python3 >/dev/null || {
        echo "build-installer: python3 is needed to check the presets are in sync." >&2
        exit 1
    }
    python3 "$ROOT/tools/make-presets.py" --check || {
        echo "build-installer: the .aupreset files are out of date. Run" >&2
        echo "    tools/make-presets.py" >&2
        echo "review the result, commit it, and build again." >&2
        exit 1
    }

    VST3_COMPANY="$(sed -n 's/.*stringCompanyName[[:space:]]*"\([^"]*\)".*/\1/p' "$ROOT/source/version.h" | head -1)"
    AU_FULLNAME="$(python3 -c 'import plistlib,sys; print(plistlib.load(open(sys.argv[1],"rb"))["AudioComponents"][0]["name"])' "$ROOT/resource/au-info.plist")"
    AU_MANUFACTURER="${AU_FULLNAME%%:*}"
    AU_PLUGIN="${AU_FULLNAME#*:}"; AU_PLUGIN="${AU_PLUGIN# }"

    [ -n "$VST3_COMPANY" ] && [ -n "$AU_MANUFACTURER" ] && [ -n "$AU_PLUGIN" ] || {
        echo "build-installer: could not read the preset folder names from" >&2
        echo "  source/version.h and resource/au-info.plist." >&2
        exit 1
    }
    ...
    mkdir -p "$WORK/root-presets/$VST3_PRESETS" "$WORK/root-presets/$AU_PRESETS"
    cp "$HERE/presets/"*.vstpreset "$WORK/root-presets/$VST3_PRESETS/" 2>/dev/null || true
    cp "$HERE/presets/"*.aupreset  "$WORK/root-presets/$AU_PRESETS/"  2>/dev/null || true

    # READABLE BY EVERYONE, set here rather than trusted. pkgbuild records the
    # modes it finds, and the files arrive owned by root: a preset copied in
    # as 600 installs as a file no user's DAW can open, and the preset menu
    # simply comes up empty. Same reasoning as the postinstall's execute bit.
...
#       -> /Users/<you>/DXi-DEv/FilterDrum-VSTi/build/VST3/Release/FilterDrum.vst3
#
...
#-----------------------------------------------------------------------------
escapes=""
# An ARRAY of roots, so a path with a space in it stays one path. The
# presets root only exists when there are presets to ship.
roots=("$WORK/root-vst3" "$WORK/root-au")
[ -d "$WORK/root-presets" ] && roots+=("$WORK/root-presets")
...
# THE POSTINSTALL IS STAGED AND RE-CHMODDED, not passed straight out of the
# repository, and this is not belt-and-braces for its own sake.
#
# pkgbuild takes --scripts exactly as it finds it. A postinstall WITHOUT its
# execute bit is never run - and the install still reports success, so the
# only symptom is that the Audio Unit does not appear until the user next
# logs out. That is a bug report you cannot reproduce.
#
# The bit is easy to lose: cp, a zip, AirDrop, a file share and an editor that
# rewrites in place have all been seen to drop it. It was in fact dropped when
# this installer was copied from Project6 - build-installer.sh and
# verify-install.sh both arrived as rw. Git records mode 755 so a clone is
# right, but a copy that never went through git is not, and this is the line
# that stops caring either way.
mkdir -p "$WORK/scripts-au"
cp "$HERE/scripts-au/postinstall" "$WORK/scripts-au/postinstall"
chmod +x "$WORK/scripts-au/postinstall"

if [ ! -x "$WORK/scripts-au/postinstall" ]; then
    echo "build-installer: could not make the postinstall executable." >&2
    echo "Shipping it unexecutable would install an Audio Unit that does not" >&2
    echo "register until the next login, and report success while doing it." >&2
    exit 1
fi
...
# The presets are data, not code, so nothing here is signed - but they ride
# inside the same notarised product archive as everything else.
if [ "$PRESET_COUNT" -gt 0 ]; then
    pkgbuild --quiet \
        --root "$WORK/root-presets" \
        --identifier "$PRESETS_ID.pkg" \
        --version "$VERSION" \
        --install-location "/Library/Audio/Presets" \
        "$WORK/pkgs/$NAME-Presets.pkg"
    echo "==> presets component built"
else
    echo "==> no presets in installer/presets - skipping that component"
fi
...
# A distribution that names a package productbuild was not given is one
# Installer cannot resolve, and it fails at the page stage naming none of
# it - so with no preset files, the block between the PRESETS markers comes
# out. Marked as a RANGE, because a multi-line XML element cannot carry a
# comment on each of its lines. (ForTran's approach, unchanged.)
if [ "$PRESET_COUNT" -eq 0 ]; then
    sed '/PRESETS-BEGIN/,/PRESETS-END/d' "$WORK/distribution.xml" > "$WORK/distribution-trimmed.xml"
    mv "$WORK/distribution-trimmed.xml" "$WORK/distribution.xml"
    echo "==> no presets, so the presets choice is not in the distribution"
fi
