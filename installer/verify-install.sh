#!/bin/bash
#-----------------------------------------------------------------------------
# Check that an INSTALLED FilterDrum actually works on this machine.
#
...
echo "FilterDrum install check - $(sw_vers -productName 2>/dev/null) $(sw_vers -productVersion 2>/dev/null) on $(uname -m)"
echo
...
#-----------------------------------------------------------------------------
receipts="$(pkgutil --pkgs 2>/dev/null | grep -i filterdrum || true)"
if [ -n "$receipts" ]; then
    while IFS= read -r r; do pass "$r"; done <<< "$receipts"
else
    note "no FilterDrum receipts - installed by hand rather than by the .pkg?"
fi
...
if command -v auval >/dev/null 2>&1; then
    ...
    if auval -v aumu FDrm AECo 2>&1 | grep -q "AU VALIDATION SUCCEEDED"; then
        pass "auval -v aumu FDrm AECo - AU VALIDATION SUCCEEDED"
    else
        fail "auval did not report success"
        note "Run it yourself for the detail:  auval -v aumu FDrm AECo"
