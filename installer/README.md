# Building the FilterDrum installer

`build-installer.sh` makes `installer/FilterDrum-<version>.pkg`, which installs
...
strings -a build/VST3/Release/FilterDrum.vst3/Contents/MacOS/FilterDrum \
...
`installer/FilterDrum-<version>.pkg` that a signed run does, so building
...
```sh
xcrun notarytool store-credentials filterdrum-notary \
    --apple-id you@example.com \
    --team-id ABCDE12345 \
    --password abcd-efgh-ijkl-mnop
```

`filterdrum-notary` is just a label you choose. The password is stored in the
...
`--notarize filterdrum-notary` stays exactly as it is.

To test the credentials without building anything:

```sh
xcrun notarytool history --keychain-profile filterdrum-notary
```
...
```sh
spctl --assess --type install -vv installer/FilterDrum-<version>.pkg
```
...
```sh
ls -ld /Library/Audio/Plug-Ins/Components/FilterDrum.component/Contents/Resources/plugin.vst3
```
...
```sh
xattr -p com.apple.quarantine /path/to/FilterDrum-<version>.pkg
```
...
```sh
xattr -w com.apple.quarantine "0081;00000000;Safari;" FilterDrum-<version>.pkg
```
...
The moment somebody else installs it, these stop being editable:
    ...
    audio.filterdrum.vst3        audio.filterdrum.audiounit
    aumu / FDrm / AECo         and the VST3 class UIDs
...
* Publish a checksum beside the asset: `shasum -a 256 FilterDrum-<version>.pkg`.
* Put the **deliberate behaviours** in the release notes — the ones a user
  will otherwise report as faults. For FilterDrum those are: Noise Level at 0
  being silence rather than a tone, note-off being ignored, Run *arming* the
  sequencer rather than starting it, and sequenced steps ignoring the Velocity
  controls. (Project6's two entries here were REAPER's handling of AU MIDI
  output and AUMIDISynth at a loop point — both were its own, and neither
  applies to a plug-in with no MIDI output.)
...
* **The binary used to carry your build paths.** VSTGUI's assert macros bake
  ...
  `-ffile-prefix-map` so they read `FilterDrum/external/...` instead. Check it
  after a rebuild:

  ```sh
  strings -a build/VST3/Release/FilterDrum.vst3/Contents/MacOS/FilterDrum       | grep -c "$HOME"        # want 0
  ```

  (`-fdebug-prefix-map` will *not* do this — it rewrites debug info and leaves
  `__FILE__` alone.)
* **The licence starts binding other people.** FilterDrum is MIT, chosen
  rather than inherited: the VST3 SDK at 3.8.1 is itself MIT throughout, so
  the old GPLv3-or-proprietary requirement does not apply and nothing upstream
  forces a licence. VSTGUI 4 is BSD-style and Apple's AudioUnitSDK is Apache
  2.0. "VST" remains a Steinberg trademark whatever the code licence, and its
  usage guidelines ship inside the SDK.
